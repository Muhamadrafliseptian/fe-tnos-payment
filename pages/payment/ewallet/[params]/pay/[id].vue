<template>
  ...
  const urlParams = router.currentRoute.value.fullPath.split("/").slice(3).join("/")
    const splitUrl = urlParams.split('/')[0]
  ...
  const getData = async () => {
    try {
      ...
      const externalId = ewalletValue(route.params.id, "external_id");
      const response = await axios.get(
        `http://localhost:3001/payment/INV-TNOS123/${route.params.id}/${externalId.value}/get`
      );
      if (paymentProcessed) {
        return;
      }
  
      const expiredDate = ewalletValue(route.params.id, "expired_date");
      ...
      if (sisaWaktu < 0) {
        ...
        router.push(`/payment/ewallet/${splitUrl}`);
      }
  ...
  const getDataUrl = () => {
    const result = ewalletValue(route.params.id, "redirectUrl");
    if (result) {
