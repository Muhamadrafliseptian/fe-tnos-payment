...
const paramsBeforeSlash = urlParams.split("/")[0]
  ...
  try {
    ...
    axios
      ...
      .then((response) => {
        ...
        transactionData.value = decryptData(response.data);
        ...
        if (sisaWaktu < 0) {
          ...
          router.push(`/payment/qrcode/${paramsBeforeSlash}`);
        }

        const decryptStatus = decryptData(response.data)

        if (decryptStatus.status !== "SUCCEEDED") {
